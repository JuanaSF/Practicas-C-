...
namespace EjemploServidor
{
    public partial class ServidorForm : Form
    {
        ...
        private void Servidor_NuevaConexion(object sender, ServidorEventArgs e)
        {
            ...
            Log($"Se ha conectado un nuevo cliente desde la IP = {e.EndPoint.Address}, Puerto = {e.EndPoint.Port}{Environment.NewLine}");
            listBox1.Items.Clear();
            foreach(var s in e.ClientesActivos)
            {
        ...
        private void Servidor_ConexionTerminada(object sender, ServidorEventArgs e)
        {
            ...
            Log($"Se ha desconectado el cliente de la IP = {e.EndPoint.Address}, Puerto = {e.EndPoint.Port}{Environment.NewLine}");
            listBox1.Items.Clear();
            foreach (var s in e.ClientesActivos)
            {
                listBox1.Items.Add(s);
        ...
        {
            // Muestro quién envió el mensaje y el mensaje
            Log($"Nuevo mensaje desde el cliente de la IP = {e.EndPoint.Address}, Puerto = {e.EndPoint.Port}  --> {e.DatosRecibidos}{Environment.NewLine}");
        ...
        {
            servidor.EnviarDatosAUno(txtMensaje.Text, listBox1.SelectedItem.ToString());
        }
    }
}
