...
namespace EjemploServidor
{
    ...
    {
        public ServidorEventArgs(IPEndPoint ep, List<string> pLC)
        {
            EndPoint = ep; ClientesActivos = pLC;
        }

        public IPEndPoint EndPoint { get; }
        public List<string> ClientesActivos { get; }
    }
    ...
    {
        public DatosRecibidosEventArgs(IPEndPoint ep, string datos) : base(ep,null)
        {
    ...
    public class Servidor
    {
        ...
        public Servidor(int puerto)
        {
            ...
            ClientesConectados= new List<string>();
        }
        ...
        private void EsperarCliente()
        {
            while (true)
            {
                ...
                CargaClientesConectados();

                // Disparo el evento NuevaConexion
                NuevaConexion?.Invoke(this, new ServidorEventArgs(endPoint,ClientesConectados));

                // Inicio el thread encargado de escuchar los mensajes del cliente
                thread.Start();
            }
        }
        private void CargaClientesConectados()
        {
            ClientesConectados.Clear();
            foreach (var x in clientes.Keys)
            {
                ClientesConectados.Add(x.ToString());
            }
        }
            ...
            while (cliente.Socket.Connected)
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        ConexionTerminada?.Invoke(this, new ServidorEventArgs(endPoint,ClientesConectados));
                        break;
                    }
                }
                catch
